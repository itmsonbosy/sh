...
function install_dependencies() {
  ...
  yum install -y \
      git ca-certificates ImageMagick postgresql-devel \
      libxml2 libxslt file gcc-c++ gcc autoconf \
      openssl-devel readline-devel zlib-devel \
      redis postgresql nodejs yarn patch ruby-devel \
      libffi-devel libyaml-devel which sudo
